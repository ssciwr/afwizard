...
# Extract the tarball
WORKDIR /opt/opals
RUN tar xzvf opals_2.5.0_linux64.tar.gz && \
    rm opals_2.5.0_linux64.tar.gz

# Copy the license file into the correct location
ADD --chown=${NB_UID} opals.key /opt/opals/opals_2.5.0/cfg

# Remove some parts of OPALS that are just not needed for our purpose but take up a lot of space
RUN rm -rf /opt/opals/opals_2.5.0/demo && \
    rm -rf /opt/opals/opals_2.5.0/doc
...
ENV OPALS_DIR=/opt/opals/opals_2.5.0
