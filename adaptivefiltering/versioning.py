...
# by adaptivefiltering. This number should be increased whenever there
...
ADAPTIVEFILTERING_DATAMODEL_MAJOR_VERSION = 0
...
ADAPTIVEFILTERING_DATAMODEL_MINOR_VERSION = 0
...
def upgrade_filter(data):
    ...
    if data["_major"] != ADAPTIVEFILTERING_DATAMODEL_MAJOR_VERSION:
        raise AdaptiveFilteringError("Loading an outdated filter")

    # If the filter is newer than the version of adaptivefiltering we also throw
    if data["_minor"] > ADAPTIVEFILTERING_DATAMODEL_MINOR_VERSION:
        raise AdaptiveFilteringError(
            "Update your version of adaptivefiltering to use this filter"
        )

    for minor in range(data["_minor"], ADAPTIVEFILTERING_DATAMODEL_MINOR_VERSION):
        data = _upgrade_functions[ADAPTIVEFILTERING_DATAMODEL_MAJOR_VERSION, minor](
            data
        )
...
#
# In the following all upgrade functions to the adaptivefiltering data model are implemented.
#
